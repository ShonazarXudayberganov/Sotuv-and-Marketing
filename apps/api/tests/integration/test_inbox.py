...
async def test_seed_mock_creates_conversations(client: AsyncClient, sample_register_payload: dict):
    ...
    convs = (await client.get("/api/v1/inbox/conversations", headers=headers)).json()
...
async def test_outbound_message_persists_and_uses_mock_telegram(
    ...
    cid = (await client.get("/api/v1/inbox/conversations", headers=headers)).json()[0]["id"]
    ...
    msgs = (await client.get(f"/api/v1/inbox/conversations/{cid}/messages", headers=headers)).json()
    assert len(msgs) == 2
    assert msgs[-1]["direction"] == "out"


async def test_mark_read_resets_unread_count(client: AsyncClient, sample_register_payload: dict):
    bundle = await _bootstrap(client, sample_register_payload)
    headers = {"Authorization": f"Bearer {bundle['access_token']}"}
    await client.post(
        "/api/v1/inbox/ingest",
    ...
    cid = (await client.get("/api/v1/inbox/conversations", headers=headers)).json()[0]["id"]
    read = await client.post(f"/api/v1/inbox/conversations/{cid}/read", headers=headers)
...
async def test_status_change_open_to_closed_back(
    ...
    refreshed = (await client.get(f"/api/v1/inbox/conversations/{cid}", headers=headers)).json()
...
async def test_draft_reply_returns_mock_response(
    ...
    cid = (await client.get("/api/v1/inbox/conversations", headers=headers)).json()[0]["id"]
    draft = await client.post(f"/api/v1/inbox/conversations/{cid}/draft-reply", headers=headers)
...
async def test_auto_reply_config_round_trip(client: AsyncClient, sample_register_payload: dict):
    bundle = await _bootstrap(client, sample_register_payload)
    headers = {"Authorization": f"Bearer {bundle['access_token']}"}
...
async def test_stats_summary(client: AsyncClient, sample_register_payload: dict):
    bundle = await _bootstrap(client, sample_register_payload)
    headers = {"Authorization": f"Bearer {bundle['access_token']}"}
