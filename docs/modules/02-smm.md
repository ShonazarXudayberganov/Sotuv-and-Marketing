...
9. 🟡 Asosiy tahlil (post statistikasi, AI o‘rganish) — analytics bor; Meta publication snapshot uchun real `likes/comments/shares` va `views/reach` pull qo‘shildi, YouTube publication snapshot uchun real `views/likes/comments` pull qo‘shildi; Telegram Bot API hozirgi integratsiyada channel post view/readback bermagani uchun production snapshot synthetic raqam to‘qimaydi va nol metric bilan qoladi; `/smm/analytics` sahifasida platforma kesimida metric source holati (`real/mixed/synthetic/unavailable`) ko‘rinadi; AI learning hali yo‘q
